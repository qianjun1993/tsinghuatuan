    ...
    if not activities.exists():
        ...
        if activity.book_end >= now:
            ...
            if tickets.exists():   # user has already booked the activity
                ticket = tickets[0]
                ticket.status = 0
		if(not ticket.seat_row is None) and (not ticket.seat_line is None):
			seats = Map.objects.filter(activity_id=activity,seat_row = ticket.seat_row,seat_line=ticket.seat_line)
			if seats.exists():
				seat = seats[0];
				seat.seat_status = 0;
				seat.save();
			ticket.seat_row = None
			ticket.seat_line = None
		ticket.save()
                Activity.objects.filter(id=activity.id).update(remain_tickets=F('remain_tickets')+1)
...
def get_reply_select_seat(msg, ticket, now, ext_desc=''):
    print ticket.activity.id
    return get_reply_single_news_xml(msg, get_item_dict(
        title=get_text_one_ticket_title(ticket, now),
        description=ext_desc + get_text_one_ticket_description(ticket, now),
        pic_url=get_text_ticket_pic(ticket),
        url=s_reverse_seat_map_detail(ticket.unique_id) #参数待改
    ))
...
def check_user_exchange(msg):
    return handler_check_text_header(msg, ['兑换'])

def response_user_exchange(msg):
    fromuser = get_msg_from(msg)
    user = get_user(fromuser)
    if user is None:
        return get_reply_text_xml(msg, get_text_unbinded_user_exchange(fromuser))
    return get_reply_text_xml(msg, get_reply_user_exchange(msg, user))

def check_user_item(msg):
    return handler_check_text_header(msg, ['查物'])

def response_user_item(msg):
    fromuser = get_msg_from(msg)
    user = get_user(fromuser)
    if user is None:
        return get_reply_text_xml(msg, get_text_unbinded_user_item(fromuser))
    return get_reply_text_xml(msg, get_reply_user_item(msg, user))
