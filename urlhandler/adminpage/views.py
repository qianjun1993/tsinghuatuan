...
def activity_create(activity):
    ...
    if preDict['seat_status'] == "1":
	preDict['total_tickets']=map.ticket1
        preDict['seat_row_total'] = map.hight1
        preDict['seat_line_total'] = map.width1
    preDict['remain_tickets'] = preDict['total_tickets']
    newact = Activity.objects.create(**preDict)
    if preDict['seat_status'] == "1":
        for i in range(0, map.hight1):
            for j in range(0, map.width1):
                Map.objects.create(activity_id=newact,seat_row=i, seat_line=j,seat_status= map.map1[i][j])
    return newact
